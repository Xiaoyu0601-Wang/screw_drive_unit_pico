...
#define GET_HIGH_ORDER_BYTE(bytes)                                             \
  ((uint8_t)((((uint16_t)(bytes)) >> 8) & 0xFF))
...
bool dynamixel2_parse_status_packet(uint8_t *packet, uint32_t packet_length,
                                    ...
                                    uint16_t *params_length, uint8_t *error,
                                    bool *crc_check);
...
bool dynamixel2_read(uint8_t id, uint16_t address, uint16_t data_length,
  ...
  dynamixel2_parse_status_packet(status_packet, status_packet_length, &id_r,
                                 return_data, return_data_length, &error,
                                 &crc_check);
...
void dynamixel2_send_packet(uint8_t id, dynamixel2_instruction_t inst,
  ...
  packet[5] =
      GET_LOW_ORDER_BYTE(params_length + 3); /* Length 1 (Low-order byte). */
  packet[6] =
      GET_HIGH_ORDER_BYTE(params_length + 3); /* Lenget 2 (High-order byte). */
  ...
  /* CRC. */
  uint16_t crc =
      update_crc(0, packet, packet_length - 2); /* Calculating CRC. */
  packet[packet_length - 2] =
      GET_LOW_ORDER_BYTE(crc); /* CRC 1 (Low-order byte). */
  packet[packet_length - 1] =
      GET_HIGH_ORDER_BYTE(crc); /* CRC 2 (High-order byte). */

  // max485_send(packet, packet_length);
  DEV_UART_Write_nByte(packet, packet_length);
}
...
bool dynamixel2_parse_status_packet(uint8_t *packet, uint32_t packet_length,
  ...
  if (packet[7] == 0x55) {
    ...
    *params_length = length - 3 - 1;
    ...
    uint16_t crc =
        update_crc(0, packet, packet_length - 2); /* Calculating CRC. */
    bool crc_l = packet[packet_length - 2] ==
                 GET_LOW_ORDER_BYTE(crc); /* CRC 1 (Low-order byte). */
    bool crc_h = packet[packet_length - 1] ==
                 GET_HIGH_ORDER_BYTE(crc); /* CRC 2 (High-order byte). */
...
bool dynamixel2_get_status_packet(uint8_t *packet, uint16_t *packet_length) {
  ...
  uint16_t length = buffer[packet_starting_index + 5] +
                    (buffer[packet_starting_index + 6] << 8);
